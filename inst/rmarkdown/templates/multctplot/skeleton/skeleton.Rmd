...
Text2 <- ifelse(EnzPlot, 
                paste0("enzyme-abundance profiles of ",
                       str_comma(unique(ct_dataframe$Enzyme)), 
                       " in ", str_comma(unique(ct_dataframe$Tissue)), "."),
                paste0(str_comma(unique(ct_dataframe$Tissue)), 
...
*Figure XXX. `r Text1` simulated `r Text2`*
...
CapText1 <-
   switch(paste(ifelse(InhibPresent, "InhibPresent", "NoInhibPresent"), 
          ...
          "NoInhibPresent EnzPlot" = ifelse(nrow(ct_dataframe %>% filter(Simulated == FALSE)) > 0,
                                            "and observed data (circles; mean of n = XXX individuals; Clinical Study XXX)", 
                                            "data (lines)"), 
