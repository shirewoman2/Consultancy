...
if(model == "all" & fitByDonor){
   ...
   # Rounding as requested - FIXME - Not working. Left off here. 
   Fit_ind <- Fit_ind %>%
   ...
   AllFits <- bind_rows(
      Fit_ind %>% mutate(across(.cols = everything(), .fns = as.character)),
      Fit_sum %>% mutate(across(.cols = everything(), .fns = as.character))) %>% 
