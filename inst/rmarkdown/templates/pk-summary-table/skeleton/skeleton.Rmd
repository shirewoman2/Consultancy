...
if(CheckDoseInt$message == "mismatch" & any(str_detect(PKpulled, "_last"))){
   ...
   #    " h, and the simulation ended at t = ", 
   #    CheckDoseInt$interval$SimDuration, 
   #    " h, so the interval for integrating the last-dose AUC was ", 
   #    CheckDoseInt$interval$IntervalRemaining, 
   #    " h rather than the dosing interval of ", 
   #    CheckDoseInt$interval$DoseInt_X, " h. Please check these results carefully.***")
   
   DoseIntWarning <- list(
      "***The time used for integrating the AUC for the last dose was not the same as the dosing interval.***",
