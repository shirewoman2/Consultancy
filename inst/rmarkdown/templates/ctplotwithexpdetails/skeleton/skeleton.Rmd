...
FigText3a <- ifelse(Deets$DoseRoute_sub == "Oral", 
...
FigText3 <- paste(FigText3a, "of", Deets$Dose_sub, "mg",
                  prettify_compound_name(Deets$Substrate))

```

Figure XXX. `r FigText1` simulated `r FigText2``r mean_type` mean `r MyTissue` concentration-time profiles after `r FigText3` in `r tidyPop(Deets$Population)$PopulationSimpleLower`. 
...
MyCompound <- prettify_compound_name(
    switch(unique(ct_dataframe$CompoundID),
...
if(InhibPresent){
    if(unique(ct_dataframe$CompoundID) %in% c("inhibitor 1", "inhibitor 2", 
                                              "inhibitor 1 metabolite")){
        CapText1a <- ifelse(nrow(ct_dataframe %>% filter(Simulated == FALSE)) > 0,
...
CapText1 <- paste(CapText1a, "for", MyCompound)
