...
if(single_table){
   ...
   # Similarly, if they only wanted 1 compoundID, don't include a column for
   # that in the table.
   ...
   if(include_dose_num == FALSE){
      names(MyPKResults) <- 
         sub("Dose 1 |Last dose |_dose1|_last", "", names(MyPKResults))
   }
   
   if(interval_in_columns){
      
      MakeBold <- list(c(0, NA), # header
                       c(NA, 1)) # 1st col
   ...
   if(as_label(shading_column) == "<empty>"){
      ...
      formatTable_Simcyp(MyPKResults,
                         ...
                         bold_cells = MakeBold, 
                         merge_columns = intersect(names(MyPKResults), 
                                                   c("File", "Interval", 
                                                     "Sheet", "Tissue", 
                                                     "CompoundID")), 
                         fontsize = fontsize,
