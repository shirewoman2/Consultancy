...
#### Simulation of `r sub("^A", "a", str_to_title(SDMD_sub_txt))` of `r Deets$Dose_sub` mg `r paste(MySubstrate, ifelse(SingMult_sub == "multiple", DoseFreq_sub, ""))` Administered with `r str_to_title(MyPerpetrator)` in `r tidyPop(Deets$Population)$PopulationCap` 
...
ugfun <- function(X){
   X <- sub("perpetrator", MyPerpetrator, X)
   return(X)
}
...
ColNames <- data.frame(OrigName = as.character(OrigPKnames)) %>% 
...
*Depicted are simulated `r CapText1`. `r CapText2` of the simulated population (n = `r Deets$NumTrials * Deets$NumSubjTrial`)`r ifelse(figure_type == "Freddy", ". The dashed lines represent the 5th and 95th percentiles", "")`. Source simulated data: `r sim_data_file`.*



*Figure XXX. Simulated `r tissue` concentration-time profiles of `r paste(Deets$Dose_inhib, "mg", DoseFreq_inhib, MyPerpetrator)` in `r tidyPop(Deets$Population)$Population`.*
...
*Depicted are simulated `r tissue` concentration-time profiles of `r MyPerpetrator` with administration of `r MySubstrate` on the `r DoseDay` day of `r NumDaysInhib` days of dosing. `r CapText3` for the simulated population (n = `r Deets$NumTrials * Deets$NumSubjTrial`). Source simulated data: `r sim_data_file`.*
