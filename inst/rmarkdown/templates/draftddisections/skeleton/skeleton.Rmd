...
#### Simulation of `r sub("^A", "a", str_to_title(SDMD_sub_txt))` of `r Deets$Dose_sub` mg `r paste(MySubstrate, ifelse(SingMult_sub == "multiple", DoseFreq_sub, ""))` Administered with `r str_to_title(MyPerpetrator)` in `r sub("Healthy Volunteers", "Healthy Subjects", tidyPop(Deets$Population)$PopulationCap)` 

`r ifelse(Deets$NumTrials %in% 1:20, Numbers[Deets$NumTrials], Deets$NumTrials)` virtual trials of `r Deets$NumSubjTrial` `r sub("healthy volunteers", "healthy subjects", tidyPop(Deets$Population)$Population)` (`r Deets$PercFemale*100`% female) aged `r Deets$Age_min` to `r Deets$Age_max` years receiving `r SDMD_sub_txt` of `r Deets$Dose_sub` mg `r MySubstrate` in the absence of `r MyPerpetrator` and `r ifelse(SingMult_sub == "multiple", "with", paste("on the", DoseDay, "day of"))` `r NumDaysInhib` days of dosing of `r MyPerpetrator` (`r Deets$Dose_inhib` mg `r DoseFreq_inhib`) were generated. `r DefaultCompound_txt` Performance verification of the `r ifelse(victim_sim, MyPerpetrator, MySubstrate)` model is provided in **Appendix A** – Performance Verification of CYP3A Substrates, Inhibitors and Inducers.
...
Mean simulated plasma `r MySubstrate` concentrations following `r SDMD_sub_txt` of `r Deets$Dose_sub` mg `r MySubstrate` in the absence of `r MyPerpetrator` and `r ifelse(SingMult_sub == "mMultiple", "with", paste("on the", DoseDay, "day of"))` `r NumDaysInhib` days of dosing of `r MyPerpetrator` (`r Deets$Dose_inhib` mg `r DoseFreq_inhib`) are illustrated in **Figure XXX**. Simulated plasma concentrations of `r MyPerpetrator` are depicted in **Figure XXX**. `r ifelse(include_enz_plot, paste0("Simulated hepatic ", enzyme, " levels in the absence and during administration of ", MyPerpetrator, " are indicated in Figure XXX. "), "")`The simulated geometric mean `r ifelse(SingMult_sub == "single", "AUC~inf~", "AUC~tau~")` and C~max~ values and corresponding GMRs for `r MySubstrate` in the absence and presence of `r MyPerpetrator` are listed in **Table XXX**.


```{r pullPK}

PKsum <- pk_table(sim_data_file = paste0(OutPath, "/", sim_data_file), 
                         ...
                         prettify_compound_names = prettify_compound_names, 
                         convert_conc_units = "ng/mL") 

pretfun <- function(X){
   if(prettify_compound_names){
      X <- sub(as.character(prettify_compound_names["perpetrator"]),
               "perpetrator", X)
   } else {
      X <- sub(MyPerpetrator, "perpetrator", X)
   }
   return(X)
}
...
if((class(prettify_compound_names) == "logical" && prettify_compound_names) |
   (class(prettify_compound_names) == "character")){
   
   names(MyPKResults) <- sub(MyPerpetrator, "perpetrator", names(MyPKResults))
   
} else {
   
   names(MyPKResults) <- sapply(names(MyPKResults), pretfun)
}

OrigPKnames <- names(MyPKResults)

PKToPull <- prettify_column_names(MyPKResults, pretty_or_ugly_cols = "ugly")
PKToPull <- names(PKToPull)
...
FigText3 <- ifelse(any(complete.cases(MyPerpetrator)),
                   paste0("with or without ", MyPerpetrator, " "),
                   "")
...
PKWeShouldHave <- switch(SingMult_sub, 
                         "multiple" = c("AUCtau_last",
                                        ...
                                        "AUCtau_ratio_last",
                                        "Cmax_ratio_last"),
                         "single" = switch(as.character(any(str_detect(PKToPull, "AUCt_dose1"))), 
                                           "TRUE" = c("AUCinf_dose1",
                                                      "AUCt_dose1",
                                                      "Cmax_dose1", 
...
TempNames <- data.frame(PKparameter = union(PKWeShouldHave, PKToPull)) %>% 
   left_join(AllPKParameters %>% select(PKparameter, SortOrder), 
             by = "PKparameter") %>% 
   arrange(SortOrder) %>% pull(PKparameter) %>% unique()

MyPKResults <- prettify_column_names(MyPKResults, 
                                     pretty_or_ugly_cols = "ugly") %>% 
   select(any_of(c("Statistic", TempNames)))
...
if(include_dose_num == FALSE){
   OrigPKnames <- sub("Dose 1 |Last dose |_dose1|_last", "", OrigPKnames)
}
...
*Table XXX. Simulated`r ifelse(Observedincluded, " and observed", "")` `r MeanTypePK` mean `r tissue` PK parameters for `r MySubstrate` after `r FigText2` of `r paste(Deets$Dose_sub, "mg", MySubstrate)` `r FigText3`in `r sub("healthy volunteers", "healthy subjects", tidyPop(Deets$Population)$Population)`.*
...
FT <- flextable::flextable(
   ...
   # Setting up to make merged header
   flextable::delete_part(part = "header")

Header <- MyPKResults[1, ]

# Baseline cols
Header[, which(str_detect(PKToPull, "_withInhib|_ratio") == FALSE)] <- 
   ifelse(is.logical(prettify_compound_names) && 
             prettify_compound_names == TRUE,
          str_to_title(MySubstrate), MySubstrate)

# + perp cols
Header[, which(str_detect(PKToPull, "_withInhib"))] <-  
   paste(ifelse(is.logical(prettify_compound_names) && 
                   prettify_compound_names == TRUE,
...
# ratio cols
Header[, which(str_detect(PKToPull, "_ratio"))] <- "GMR"

# Statistic should be blank
Header[, 1] <- "" 

FT <- FT %>%
   flextable::add_header(values = as.list(Header)) %>% 
   flextable::merge_h(part = "header") %>%
   flextable::add_body(values = as.data.frame(t(
      # need to remove "with perpetrator" b/c redundant
      sub(" with perpetrator", "", OrigPKnames)))) %>%
...
if(include_dose_num){
   ...
   ColNames <- data.frame(OrigName = OrigPKnames) %>% 
   ...
   ColNames <- data.frame(OrigName = OrigPKnames) %>% 
      mutate(Part1 = ifelse(str_detect(OrigName, NamePattern), 
                            "", OrigName), 
             Part2 = sub("inf|tau|max|min|a|g|h|lag", "", str_trim(str_extract(OrigName, NamePattern))), 
             Part2 = sub("AUCt", "AUC", Part2),
             Subscript = sub("^AUC|^C|^t|^f|^k", "", str_extract(OrigName, NamePattern)), 
   ...
   if(complete.cases(ColNames$Subscript[i])){
      FT <- FT %>% 
         flextable::compose(part = "body", i = 1,
                            j = which(OrigPKnames == ColNames$OrigName[i]),
...
names(MyPKResults) <- as.character(OrigPKnames)
...
`r ifelse(length(MissingPK) > 0, "***WARNING: There were problems extracting some PK values, e.g., problems with extrapolation to infinity. Please check your results.***", "")`
...
*Figure XXX. `r Text1` simulated `r tissue` concentration-time profiles of `r MySubstrate` `r Text2`in `r sub("healthy volunteers", "healthy subjects", tidyPop(Deets$Population)$Population)`.*
...
ct_plot(ct_dataframe %>% filter(File == sim_data_file & Tissue == tissue,
        ...
        line_width = line_width_sub, 
        hline_position = hline_position_sub,
        hline_style = hline_style_sub, 
        vline_position = vline_position_sub, 
        vline_style = vline_style_sub)
...
CapText1 <- paste(
   ...
   "in the absence of", MyPerpetrator, "(solid line)",
...
CapText2 <- switch(
   ...
   "trial means" = 
      paste("The grey lines represent",
            ...
            "mean data"), 
   "percentiles" = 
      paste("The lighter lines represent the 5^th^ and 95^th^ percentiles and the darker lines the",
            mean_type_graph, 
            "mean data"), 
   "percentile ribbon" = 
      paste("The shaded regions represent the 5^th^ to the 95^th^ percentiles and the lines the",
...
NextChunkHeight <- ifelse(linear_or_log_inhib %in% c("both", "both vertical"), 6, 3)

```

*Depicted are simulated `r CapText1`. `r CapText2` of the simulated population (n = `r Deets$NumTrials * Deets$NumSubjTrial`)`r ifelse(figure_type == "Freddy", ". The dashed lines represent the 5^th^ and 95^th^ percentiles", "")`. Source simulated data: `r basename(sim_data_file)`.*



*Figure XXX. Simulated `r tissue` concentration-time profiles of `r paste(Deets$Dose_inhib, "mg", DoseFreq_inhib, MyPerpetrator)` in `r sub("healthy volunteers", "healthy subjects", tidyPop(Deets$Population)$Population)`.*
...
ct_plot(ct_dataframe %>% filter(File == sim_data_file & Tissue == tissue,
        ...
        line_width = line_width_inhib, 
        hline_position = hline_position_inhib,
        hline_style = hline_style_inhib, 
        vline_position = vline_position_inhib, 
        vline_style = vline_style_inhib)
...
*Depicted are simulated `r tissue` concentration-time profiles of `r MyPerpetrator` with administration of `r MySubstrate` on the `r DoseDay` day of `r NumDaysInhib` days of dosing. `r CapText3` for the simulated population (n = `r Deets$NumTrials * Deets$NumSubjTrial`). Source simulated data: `r basename(sim_data_file)`.*
