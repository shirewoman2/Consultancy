...
SDMD_sub_txt <- paste(switch(exp_detail_data$Regimen_sub, 
                       "custom dosing" = "multiple", 
                       "Multiple Dose" = "multiple", 
                       "Single Dose" = "a single"), 
                      
                      switch(exp_detail_data$DoseRoute_sub, 
                             "custom dosing" = "CUSTOM DOSING - CHECK THIS", 
                             "Oral" = "oral", 
                             "IV" = "IV"), 
                      
                      switch(exp_detail_data$Regimen_sub, 
                       "custom dosing" = "doses", 
                       "Multiple Dose" = "doses", 
                       "Single Dose" = "dose")
                      )
...
SDMD_inhib_txt <- paste(switch(exp_detail_data$Regimen_inhib, 
                             "custom dosing" = "multiple", 
                             "Multiple Dose" = "multiple", 
                             "Single Dose" = "a single"), 
                      ...
                      switch(exp_detail_data$Regimen_inhib, 
                             "custom dosing" = "doses", 
                             "Multiple Dose" = "doses", 
                             "Single Dose" = "dose")
)
...
suppressMessages(
    PKToPull <- data.frame(
        ...
        filter(complete.cases(PKparameter) &
                  !str_detect(PKparameter, "PercExtrap")) %>% pull(PKparameter) %>%
       unique()
)
...
if(ncol(MyPKResults) != 7){
   PKWeShouldHave <- switch(exp_detail_data$Regimen_sub, 
                       "Multiple Dose" = c("AUCtau_last","Cmax_last", 
                                           "AUCtau_last_withInhib", "Cmax_last_withInhib", 
                                         "AUCtau_ratio_last", "Cmax_ratio_last"),
                       "Single Dose" = c("AUCinf_dose1", "Cmax_dose1", 
                                         "AUCinf_dose1_withInhib", "Cmax_dose1_withInhib", 
                                         "AUCinf_ratio_dose1", "Cmax_ratio_dose1"))
   MissingPK <- setdiff(PKWeShouldHave, PKToPull)
   ...
   TempNames <- data.frame(
      PrettifiedNames1 = names(MyPKResults), 
      PrettifiedNames = sub(as.character(prettify_compound_names["effector"]),
                                         "effector", names(MyPKResults))) %>%
      left_join(AllPKParameters, multiple = "all", by = join_by(PrettifiedNames)) %>%
      mutate(PKparameter = ifelse(is.na(PKparameter), PrettifiedNames, PKparameter)) %>% 
      select(PrettifiedNames1, PKparameter) %>% unique() %>% 
      mutate(PKparameter = factor(PKparameter, levels = c("Statistic", 
                                                          PKWeShouldHave))) %>% 
      arrange(PKparameter)
   
   MyPKResults <- MyPKResults[, TempNames$PrettifiedNames1]
...
FT <- flextable::flextable(MyPKResults %>% 
                              mutate(Statistic = ifelse(V1 == "Simulated", 
                                                        paste(str_to_title(MeanTypePK), "mean"), V1))) %>%
