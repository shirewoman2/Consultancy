...
#### Simulation of `r sub("^A", "a", str_to_title(SDMD_sub_txt))` of `r Deets$Dose_sub` mg `r paste(ifelse(prettify_compound_names, str_to_title(MySubstrate), MySubstrate), ifelse(SingMult_sub == "multiple", DoseFreq_sub, ""))` Administered with `r str_to_title(MyPerpetrator)` in `r tidyPop(Deets$Population)$PopulationCap` 
...
if((class(prettify_compound_names) == "logical" && prettify_compound_names) |
   ...
   PKToPull_DF <- data.frame(
      PrettifiedNames = sub(MyPerpetrator, "perpetrator", names(PKsum$Table)))
} else {
   PKToPull_DF <- data.frame(
      PrettifiedNames = sapply(names(PKsum$Table), pretfun)) 
}

PKToPull_DF <- PKToPull_DF %>% 
