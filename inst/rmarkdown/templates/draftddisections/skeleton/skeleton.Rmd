...
SingMult_sub <- ifelse(exp_detail_data$Regimen_sub %in% c("custom dosing",
                                                          "Multiple Dose"), 
                       "multiple", "single")
SingMult_inhib <- ifelse(exp_detail_data$Regimen_inhib %in% c("custom dosing",
                                                              "Multiple Dose"), 
                         "multiple", "single")
...
DoseFreq_sub <- switch(as.character(exp_detail_data$DoseInt_sub),
                       "12" = "BID", 
                       "24" = "QD", 
...
DoseFreq_inhib <- switch(as.character(exp_detail_data$DoseInt_inhib),
                         "12" = "BID", 
                         "24" = "QD", 
                         "8" = "three times per day", 
                         "6" = "four times per day", 
...
NumDaysInhib <- suppressWarnings(
   exp_detail_data$NumDoses_inhib*as.numeric(exp_detail_data$DoseInt_inhib)/24)
...
#### Simulation of `r sub("^A", "a", str_to_title(SDMD_sub_txt))` of `r exp_detail_data$Dose_sub` mg `r paste(str_to_title(MySubstrate), ifelse(SingMult_sub == "multiple", DoseFreq_sub, ""))` Administered with `r str_to_title(MyEffector)` in `r tidyPop(exp_detail_data$Population)$PopulationCap` 

`r ifelse(exp_detail_data$NumTrials %in% 1:20, Numbers[exp_detail_data$NumTrials], exp_detail_data$NumTrials)` virtual trials of `r exp_detail_data$NumSubjTrial` `r tidyPop(exp_detail_data$Population)$Population` (`r exp_detail_data$PercFemale*100`% female) aged `r exp_detail_data$Age_min` to `r exp_detail_data$Age_max` years receiving `r SDMD_sub_txt` of `r exp_detail_data$Dose_sub` mg `r MySubstrate` in the absence of `r MyEffector` and `r ifelse(SingMult_sub == "multiple", "with", paste("on the", DoseDay, "day of"))` `r NumDaysInhib` days of dosing of `r MyEffector` (`r exp_detail_data$Dose_inhib` mg `r DoseFreq_inhib`) were generated. `r DefaultCompound_txt` Performance verification of the `r ifelse(victim_sim, MyEffector, MySubstrate)` model is provided in Appendix A – Performance Verification of CYP3A Substrates, Inhibitors and Inducers.
...
FigText2 <- ifelse(Dose1included & LastDoseincluded, 
                   ifelse(exp_detail_data$DoseRoute_sub == "Oral", 
                          ...
                          paste("the first and multiple", 
                                exp_detail_data$DoseRoute_sub, "doses")),
...
*Depicted are simulated `r tissue` concentration-time profiles of `r MyEffector` with administration of `r MySubstrate` on the `r DoseDay` day of `r NumDaysInhib` days of dosing. `r CapText3` for the simulated population (n = `r exp_detail_data$NumTrials * exp_detail_data$NumSubjTrial`). Source simulated data: `r sim_data_file`.*
