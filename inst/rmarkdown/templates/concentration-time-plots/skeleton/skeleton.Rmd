...
if(length(MySubstrate) > 0){
   ...
   Text2 <- paste("XXX mg", MyPerpetrator, "QD/BID/TID ")
   MyCompound <- ""
}
   ...
   CapText1 <-
      switch(as.character(
         ...
         "TRUE" = paste(
            ...
            "in the absence of", prettify_compound_name(MyPerpetrator), 
            " (solid line) and on the XXth day of XX days of dosing of", 
            prettify_compound_name(MyPerpetrator), "(dashed line)"))
} else {
   ...
   CapText1 <- paste(
      ifelse(nrow(ct_dataframe %>% filter(Simulated == FALSE)) > 0,
             paste("and observed (circles;", mean_type, 
                   "mean of n = XXX individuals; Clinical Study XXX)"), 
             ""), 
      MyTissue, "concentration-time profiles of", MyPerpetrator, 
