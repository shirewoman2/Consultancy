...
- Make typical PK tables
  ...
  - *pksummary_mult() - Soon to be deprecated in favor of the more
    versatile pk_table()* - Make PK summary tables from multiple
    simulator output files at once
  - *pksummary_table() - Soon to be deprecated in favor of the more
    versatile pk_table()* - Make a summary PK table for a report or
    slide deck
- Make PK tables that make comparisons not natively available in the
  Simulator outputs
...
- Calculate PK
  ...
  - elimFit() - Calculate the elimination rate(s) for a
...
- Helper functions for PK tables
  ...
  - extractPK() - Extract PK data for specific parameters from a
  ...
  - *getSectionInfo() - No longer being developed* - Get information
    from an Excel report-input file about a section for a report
...
- Read or change workspaces - Most functions that actually change
  workspaces are only available in the beta version of the package.
  ...
  - make_xml_path_mine() - Make the user name in the path of observed
    data overlay XML files and fixed trial design XML files match that
    of the current user
- Helper functions that call on the Simcyp package
  - check_simulator_initialized() - check whether the Simcyp Simulator
    has been initialized

## Miscellany

- Fitting induction models to in vitro data
...
- List all the possible DDIs in a simulation
...
- Tidbits for making quick calculations
  - calcKi() - Calculate the Ki of an inhibitor with the Cheng-Prusoff
    equation
  - centerBin() - Cut a set of numeric data into bins based on the
    middle value of the bin
  ...
  - countWords() - Useful for guessing at good widths for columns in
    Excel output
  ...
  - str_comma() - Collapse a vector of character strings into one
    readable string, separated by commas and by “and” and “or”
