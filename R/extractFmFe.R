...
#'   dynamic fm and fe data, in quotes
...
extractFmFe <- function(sim_data_file,
   ...
   if(any(c("aggregate", "both") %in% returnAggregateOrIndiv)){
      ...
      if(all(complete.cases(Tissues))){
         ...
         NamesToCheck <- tolower(sim_data_xl$...1[TimeRow[1]:(FirstBlank-1)])
         
         # Need to note which rows are for which tissue.
         TissueRows <- list(
            "liver" = which(str_detect(NamesToCheck, "\\(liver\\)")) +
               TimeRow - 1,
            "kidney" = which(str_detect(NamesToCheck, "\\(kidney\\)")) +
               TimeRow-1, 
            "renal" = which(str_detect(NamesToCheck, "\\(renal\\)")) +
               TimeRow-1)
         ...
         for(i in Tissues){
            ...
            sim_data_mean[[i]] <- list()
            
            # There may be more than one enzyme when the tissue isn't "renal". 
            Enzymes <- as.character(
               sapply(sim_data_xl[TissueRows[[i]], 1], 
                      function(x) str_extract(x, "(CYP|UGT)[1-3][ABCDEJ][1-9]{1,2}")))
            
            for(j in unique(Enzymes)){
               # Checking which cells contain mean, 5th, and 95th
               # percentile data.
               EnzRows <- TissueRows[[i]][
                  which(str_detect(sim_data_xl$...1[TissueRows[[i]]], j))]
               NamesToCheck <- tolower(as.character(t(sim_data_xl[EnzRows, 1])))
               ...
               names(sim_data_mean[[i]][[j]]) <- c("Time", names(RowsToUse))
               sim_data_mean[[i]][[j]] <- sim_data_mean[[i]][[j]] %>%
                  pivot_longer(names_to = "Trial", values_to = "FmOrFe", 
                               cols = -Time) %>%
                  mutate(Enzyme = j,
                         Tissue = i, 
                         ValType = )
               ...
               if(EffectorPresent){
                  ...
                  # Checking which cells contain mean, 5th, and 95th
                  # percentile data.
                  NamesToCheck <- tolower(sim_data_xl$...1[TissueRows[[i]]])
                  ...
                  sim_data_mean_inhib <- sim_data_xl[c(TissueRows[[i]][1], RowsToUse), ] %>%
                     t() %>%
                     as.data.frame() %>% slice(-(1:3)) %>%
                     mutate_all(as.numeric)
      ...
      if(any(c("individual", "both") %in% returnAggregateOrIndiv)){
         ...
         Tissues <- c("colon", "small intestine")[
            c(any(str_detect(tolower(sim_data_xl$...1), "\\(colon\\)")),
              any(str_detect(tolower(sim_data_xl$...1), "\\(si\\)|\\(gut\\)")))]
         
         if(length(which(complete.cases(Tissues))) > 0 & tissue == "gut"){
            
            sim_data_ind <- list()
            
            StartRow_ind <- which(sim_data_xl$...1 == "Individual Statistics")
            TimeRow <- which(str_detect(sim_data_xl$...1, "^Time "))
            TimeRow <- TimeRow[TimeRow > StartRow_ind][1:2]
            
            # Figuring out which rows contain which data
            FirstBlank <- intersect(which(is.na(sim_data_xl$...1)),
                                    which(1:nrow(sim_data_xl) > TimeRow[2]))[1]
            FirstBlank <- ifelse(is.na(FirstBlank), nrow(sim_data_xl), FirstBlank)
            NamesToCheck <- tolower(sim_data_xl$...1[TimeRow[1]:nrow(sim_data_xl)])
            
            # Need to note which rows are for which gut part.
            LiverRows <- which(str_detect(NamesToCheck, "\\(si\\)|\\(gut\\)")) +
               TimeRow[1]-1
            LiverTimeRows <- intersect(TimeRow+1, LiverRows) - 1
            # Looking for the next blank row after LiverTimeRows
            SIEndRow <- which(is.na(sim_data_xl$...1))
            SIEndRow <- SIEndRow[SIEndRow > LiverTimeRows][1] - 1
            ...
            KidneyRows <- which(str_detect(NamesToCheck, "\\(colon\\)")) +
               TimeRow[1]-1
            ColonTimeRow <- intersect(TimeRow+1, KidneyRows) - 1
            ...
            TissueRows <- list("colon" = ColonTimeRow:ColonEndRow,
                               "small intestine" = LiverTimeRows:SIEndRow)
            ...
            for(i in Tissues){
               ...
               RowsToUse <- intersect(
                  TissueRows[[i]],
                  TissueRows[[i]][which(!str_detect(tolower(sim_data_xl$...1[TissueRows[[i]]]),
                                                    "with inh"))])
               ...
               sim_data_ind[[i]] <- sim_data_ind[[i]] %>%
                  pivot_longer(names_to = "SubjTrial", values_to = "FmOrFe",
                               cols = -Time) %>%
                  mutate(Enzyme = enzyme,
                         Tissue = i,
               ...
               if(EffectorPresent){
                  ...
                  RowsToUse <- TissueRows[[i]][which(
                     str_detect(tolower(sim_data_xl$...1[TissueRows[[i]]]),
                                "with inh"))]
                  
                  sim_data_ind_inhib <- sim_data_xl[c(TissueRows[[i]][1], RowsToUse), ] %>%
                     t() %>%
                     as.data.frame() %>% slice(-(1:3)) %>%
                     mutate_all(as.numeric)%>%
                     rename(Time = "V1")
