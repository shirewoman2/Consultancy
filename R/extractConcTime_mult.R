...
#' comparisons. \strong{Note:} We plan to expand this function to include
#' options such as pulling multiple \emph{kinds} of data from a \emph{single}
#' output file so that you could, for example, overlay bound and unbound plasma
#' drug concentrations or blood vs. plasma concentrations, etc., but that is
#' still under construction. -LS
#'
...
#'   \code{conctime_DF}. Since pulling data from Excel files is slow, by default,
#'   this will \emph{not} overwrite existing data and instead will only add data
#'   from any Excel files that aren't already included. A situation where you
#'   might want to set this to TRUE would be when you have changed input
#'   parameters for simulations and re-run them.
