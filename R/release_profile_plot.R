...
release_profile_plot <- function(existing_exp_details, 
   ...
   MyCompounds <- existing_exp_details$MainDetails %>% 
      ...
      pivot_longer(cols = -File, 
                   ...
                   values_to = "Compound") %>% 
      left_join(AllCompounds, by = "DetailNames")
      
   Release <- existing_exp_details$ReleaseProfiles %>% 
      mutate(ReleaseUpper = ReleaseMean + ReleaseMean * ReleaseCV, 
             ReleaseLower = ReleaseMean - ReleaseMean * ReleaseCV, 
             ReleaseSD = ReleaseMean * ReleaseCV, 
             subsection_ADAM = NA, 
             Simulated = TRUE, 
             # placeholders only
             Inhibitor = "none",
   ...
   # ggplot(Release, aes(x = Time, y = ReleaseMean, 
