...
difftime_sim <- function(time1, time2, units = "hours"){
    ...
    Hrs_t1 <- as.numeric(sub(":[0-9]{2}", "",
                             stringr::str_extract(time1, "[0-9]{1,2}:[0-9]{2}")))
    Mins_t1 <- as.numeric(sub("[0-9]{1,2}:", "",
                              stringr::str_extract(time1, "[0-9]{1,2}:[0-9]{2}")))
    
    Days_t2 <- as.numeric(sub("Day ", "",
                              stringr::str_extract(time2, "Day [0-9]{1,}"))) - 1
    Hrs_t2 <- as.numeric(sub(":[0-9]{2}", "",
                             stringr::str_extract(time2, "[0-9]{1,2}:[0-9]{2}")))
    Mins_t2 <- as.numeric(sub("[0-9]{1,2}:", "",
                              stringr::str_extract(time2, "[0-9]{1,2}:[0-9]{2}")))
