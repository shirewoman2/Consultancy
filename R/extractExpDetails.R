...
extractExpDetails <- function(sim_data_file,
    ...
    if(any(str_detect(exp_details, "StartDayTime")) & 
       ...
       any(CustomDosing, na.rm = TRUE)){
        ...
        # PossCustDose <- data.frame(
        #     CompoundID = c("substrate", "inhibitor 1", "inhibitor 2"),
        #     Compound = c(Out$Substrate, 
        #                  ifelse(is.null(Out$Inhibitor1), NA, Out$Inhibitor1),
        #                  ifelse(is.null(Out$Inhibitor2), NA, Out$Inhibitor2)),
        #     Suffix = c("_sub", "_inhib", "_inhibitor2"),
        #     TextString = c("Custom Dosing - Sub", 
        #                    "Custom Dosing - Inh 1", 
        #                    "Custom Dosing - Inh 2")) %>% 
