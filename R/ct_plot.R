...
#' observed data were in the presence of an effector,
#' \code{\link{extractConcTime}} cannot tell the difference and will thus assume
#' all observed data included in the simulator output were for the substrate in
#' the \emph{absence} of any effector. If you want to specify which compound the
#' observed data were for and/or whether any effectors were present, when you
#' extract your data using the function \code{\link{extractConcTime}}, please
#' supply an observed data file to the argument \code{obs_data_file}.} \item{If
#' you attempt to use data generated by \code{\link{extractConcTime_mult}} here,
#' you can get weird results if you're not careful to only include one tissue
#' and one compound ID. If you see something odd, try including the legend to
#' help decipher what might have gone awry.}}
#'
#' @param sim_obs_dataframe the input concentration-time data generated by
#'   running the function \code{\link{extractConcTime}}. (If you supply
#'   concentration-time data generated from \code{\link{extractConcTime_mult}}
#'   instead, you'll get a weird looking graph.)
...
ct_plot <- function(sim_obs_dataframe = NA,
    ...
    compoundToExtract <- ifelse(EnzPlot, sort(unique(Data$Enzyme)), 
                                sort(unique(Data$CompoundID)))
    if(EnzPlot){
    ...
    MyEffector <- unique(sort(Data$Inhibitor)) %>% as.character()
    ...
    # Checking on whether the observed data were extracted with the simluator
    # output alone b/c we don't know what the compound ID was or whether an
    # inhibitor were present. Giving a warning when that's the case.
    if(nrow(obs_data) > 0 && any(is.na(obs_data$CompoundID)) | 
       any(is.na(obs_data$Inhibitor))){
        warning("You appear to have supplied data where the observed data were included in the simulator output. Please be aware that the simulator output doesn't include information about which compound the observed data represent or whether there was an inhibitor present.")
    }
    ...
    if(str_detect(figure_type, "percentile") &&
       !str_detect(figure_type, "ribbon")){
        ...
        if(length(MyEffector) > 0 && complete.cases(MyEffector[1]) &&
           MyEffector[1] != "none" & compoundToExtract != "inhibitor 1"){
            # Graph for when effector is present
            
            warning("When there is an effector present in the simulation, as is the case here, the Simcyp Consultancy report template recommends only showing the means. You may want to change figure_type to 'means only'.")
            
            A <- ggplot(sim_data_mean %>%
                            filter(Trial %in% c("per5", "per95")) %>%
                            mutate(Group = paste(Group, Trial, Inhibitor)),
                        aes(x = Time, y = Conc,
                            linetype = Inhibitor, shape = Inhibitor,
                            color = Inhibitor, fill = Inhibitor, 
                            group = Group)) +
            ...
            # Graph for when effector is absent
            
            ## linear plot
            A <- ggplot(sim_data_mean %>% filter(Trial != MyMeanType) %>% 
                            mutate(Group = paste(Trial, Inhibitor)),
                        aes(x = Time, y = Conc, group = Group)) +
                geom_line(alpha = AlphaToUse, 
                          lwd = ifelse(is.na(line_width), 0.8, line_width),
                          linetype = line_type[1],
           ...
           MyEffector[1] != "none" & compoundToExtract != "inhibitor 1"){
            ...
            if(nrow(obs_data) > 0){
                ...
                if(obs_color[1] == "none"){
                    ...
                    # This is when they have either accepted the default color
                    # for figure_type Freddy or they have specified a color.
                    A <- A +
                        geom_point(data = obs_data, size = 2,
                                   alpha = 0.5, stroke = 1) +
                        scale_fill_manual(values = obs_color) +
    ...
    suppressMessages(suppressWarnings(return(Out)))
}
