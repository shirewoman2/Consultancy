...
#'   \emph{all} observed data goes with \emph{all} simulated data. To specify,
#'   use a data.frame like this:
...
match_obs_to_sim <- function(ct_dataframe, 
   ...
   if(all(is.na(obs_to_sim_assignment))){
      ...
      # SCENARIO A: All obs data should match all sim data. 
      
      # FIXME - need to assign compound still. need to address possibility that ct_dataframe contains more than one compound
      
      # If there are no values assigned for File and the user did not specify
      # anything for obs_to_sim_assignment, then make all the observed data go
      # with all the simulated data.
      FileAssign <- expand_grid(ObsFile = obs_dataframe %>% pull(ObsFile) %>% unique(), 
                                File = unique(ct_dataframe$File)) %>% 
         filter(complete.cases(File))
      suppressMessages(
         obs_dataframe <- FileAssign %>% full_join(obs_dataframe %>% select(-File))
      )
   } else {
      
      # SCENARIO B: Specific obs files should match specific sim files.
      ...
      obs_dataframe <- bind_rows(obs_dataframe) %>% unique()
      
   }
   
   ct_dataframe <- ct_dataframe %>% bind_rows(obs_dataframe) %>% 
