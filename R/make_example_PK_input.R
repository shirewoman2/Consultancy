...
make_example_PK_input <- function(){
   ...
   Opts_ratios <- c("1" = wrapn("everything is the same except the simulation files, e.g., you're comparing fasted vs. fed or healthy vs. hepatic impairment and want standard PK parameters"), 
                    ...
                    "6" = wrapn("get the data from different user-specified AUC intervals for the numerator and denominator sims, e.g., you want to compare the last-dose PK between two simulations but one simulation included a washout period and the other did not, which means you need a user-specified interval sheet for one and the regular last-dose sheet for the other"))
   
   message(str_wrap("Would you like examples for standard PK parameters, e.g., for the function 'pk_table', or for ratios of PK parameters from different simulations, e.g., for the functions 'calc_PK_ratios' or 'estimate_sample_size'?"))
