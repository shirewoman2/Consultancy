...
#' \code{forest_dataframe}. In the graph, the data will be broken up by: 1) the
#' file name, which will be graphed on either the main y axis or, if you would
#' like, a secondary y axis and 2) anything you specify for
...
forest_plot <- function(forest_dataframe, 
    ...
    # versions and also noting which data was simulated
    forest_dataframe <- forest_dataframe %>% 
        rename(PerpCompound = any_of("Inhibitor1"), 
               VictimCompound = any_of("Substrate")) %>% 
    ...
    if(as_label(y_axis_column_secondary) != "<empty>"){
        ...
        if(as_label(facet_column_x) == "PKparameter"){
            ...
            G <- ggplot(forest_dataframe, aes(x = Centre, xmin = Lower, xmax = Upper, 
                ...
                geom_point(size = 2.5, fill = "white") +
                scale_shape_manual(values = MyShapes) +
                scale_y_discrete(expand = expansion(mult = pad_y_num/10)) + # pad_y_num should be smaller w/discrete scale
                labs(fill = "Interaction level", shape = NULL) +
