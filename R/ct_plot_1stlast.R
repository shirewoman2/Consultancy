...
#'
#' @description \code{ct_plot_1stlast} makes concentration-time plots like
#' of the first and last doses of a simulation or 
#'
...
#'
#' @examples
#' # We'll use some example multiple-dose concentration-time data
#' # included in the package for this example: 
#' ct_plot_1stlast(ct_dataframe = LMVct)
#' 
#' 
...
ct_plot_1stlast <- function(ct_dataframe, 
   ...
   if(length(sort(unique(ct_dataframe$DoseNum))) == 1){
      stop(wrapn("ct_plot_1stlast is only for multiple-dose scenarios, but these data appear to be for only one dose."))
   }
   ...
   if(all(is.na(ct_dataframe$DoseNum))){
      stop(wrapn("All the dose numbers in your data are NA, and we need to know which time ranges were the 1st and which were the last doses, so we cannot make a graph with the current data. Please check your input for ct_dataframe."), 
           call. = FALSE)
   }
